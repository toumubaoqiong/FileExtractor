...
blargg_err_t File_Extractor::open_arc_file( bool unbuffered )
{
    ...
    if ( err )
    {
		//FE_PRINT("[open_arc_file]delete  in = 0x%x\n", in);
        delete in;
		in = (Std_File_Reader*)NULL;
    }
    else
    {
        reader_ = in;
...
{
	//FE_PRINT("[File_Extractor 0x%x::close_]delete  own_file_ = 0x%x\n", this, own_file_);
    delete own_file_;
    own_file_ = (File_Reader*)NULL;

    tell_   = 0;
    reader_ = (File_Reader*)NULL;
    opened_ = false;
...
File_Extractor::~fex_t()
{
	...
	//FE_PRINT("[File_Extractor 0x%x::~fex_t]delete  own_file_ = 0x%x\n", this, own_file_);
...
{
	name_       = (const char*)NULL;
	wname_      = (const wchar_t*)NULL;
	done_       = true;
	stat_called = false;
	data_ptr_   = NULL;
...
blargg_err_t File_Extractor::extract_v( void* out, int count )
{
	void const* p;
	RETURN_ERR( data( &p ) );
	memcpy( (char*)out, STATIC_CAST(char const*,p) + (size() - remain()), count );
