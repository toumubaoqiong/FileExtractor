...
blargg_err_t Remaining_Reader::read_v( void* out, int count )
{
	...
	if ( first )
	{
		memcpy( (char*)out, (char*)header, first );
...
{
	memcpy( (char*)p, (char*)(begin + tell()), s );
	return blargg_ok;
}
...
static inline FS_FILE* blargg_fopen( const char path [], const char mode [] )
{
	//printf("blargg_fopen: %s %s\n", path, mode );
	return fs_fopen( (char*)path, (char*)mode );
}
...
static blargg_err_t blargg_fopen( FS_FILE** out, const char path [] )
{
	errno = 0;
	*out = blargg_fopen( path, "rb" );
	if ( NULL == *out )
	{
		printf("blargg_fopen_ faild.\n");
		#ifdef ENOENT
			if ( errno == ENOENT )
	...
	printf("blargg_fopen_ success.\n");

	return blargg_ok;
}

static blargg_err_t blargg_fsize( FS_FILE* f, long* out )
{
	if ( fs_fseek( f, 0, SEEK_END ) )
...
blargg_err_t Std_File_Reader::open( const char path [] )
{
	...
	if ( err )
	{
		FE_PRINT( "Std_File_Reader::open blargg_fsize error\n" );
		fs_fclose( f );
		return err;
	}
...
//Lshain�޸� setvbuf�޴˺��� �������
#if 0
	if ( setvbuf( STATIC_CAST(FILE*, file_), (char*)NULL, _IONBF, 0 ) )
...
{
	if ( fs_fseek( (FS_FILE*)file_, n, SEEK_SET ) )
	{
#if 0
		// Data_Reader's wrapper should prevent EOF
		check( !feof( STATIC_CAST(FILE*, file_) ) );
