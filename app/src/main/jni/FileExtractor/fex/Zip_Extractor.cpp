	...
	File_Extractor( fex_zip_type )
{
	...
	assert( offsetof (header_t,filename)   == (unsigned int)header_size );
	assert( offsetof (entry_t,filename)    == (unsigned int)entry_size );
	assert( offsetof (end_entry_t,comment) == (unsigned int)end_entry_size );
}
...
blargg_err_t Zip_Extractor::first_read( int count )
{
	...
	crc = ::crc32( 0, (const Bytef*)NULL, 0 );
