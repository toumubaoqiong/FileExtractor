...
BLARGG_EXPORT fex_err_t fex_open_type( fex_t** fe_out, const char path [], fex_type_t type )
{
    *fe_out = (fex_t*)NULL;
    ...
    if ( err )
    {
		FE_PRINT("[fex_open_type]delete  fe = 0x%x\n", fe);
        delete fe;
		fe = (fex_t*)NULL;
...
BLARGG_EXPORT void        fex_close           ( fex_t* fe )                         { /*FE_PRINT("[fex_close]delete  fe = 0x%x\n", fe);*/ delete fe; fe = (fex_t*)NULL; }
