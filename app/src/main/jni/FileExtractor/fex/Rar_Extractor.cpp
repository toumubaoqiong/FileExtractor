...
extern "C"
{
	static unrar_err_t my_unrar_read( void* data, void* out, int* count, unrar_pos_t pos )
	{
		...
		if ( h->pos != pos )
		{
			blargg_err_t err = h->in->seek( (int)pos );
			if ( err )
...
blargg_err_t Rar_Extractor::skip_unextractables()
{
	...
	if ( !unrar_done( unrar ) )
	{
		...
		set_name( info->name, (info->name_w && *info->name_w) ? info->name_w : (const wchar_t*)NULL );
		set_info( (int)info->size, info->dos_date, (info->is_crc32 ? info->crc : 0) );
	}
...
{
	return (fex_pos_t)unrar_tell( unrar );
}
