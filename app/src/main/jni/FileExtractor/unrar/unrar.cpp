...
unrar_t::unrar_t() :
	Buffer( &Arc )
{
	Arc.user_read   = (unrar_read_func)NULL;
	Arc.user_write  = (unrar_write_func)NULL;
	Arc.Tell_       = 0;
	Arc.write_error = unrar_ok;
	data_           = NULL;
	own_data_       = NULL;
	close_file      = (Close_File_Fun)NULL;
	FileCount       = 0;
	Unp             = (Unpack*)NULL;
...
unrar_t::~unrar_t()
{
	...
	if ( Unp )
	{
		FE_PRINT("[unrar_t 0x%x::~unrar_t]delete  Unp = 0x%x\n", this, Unp);
		delete Unp;
		Unp = (Unpack*)NULL;
	}
...
static unrar_err_t extract_( unrar_t* p, unrar_write_func user_write, void* user_data )
{
	...
	RETURN_ERR( p->ExtractCurrentFile( user_write == (unrar_write_func)NULL ) );
	p->Arc.user_write      = (unrar_write_func)NULL;
...
static unrar_err_t skip_solid( unrar_t* p )
{
	...
	return extract_( p, (unrar_write_func)NULL, NULL );
}
...
//// Interface
	...
	struct unrar_ptr 
	{
		...
		{ 
			if ( p )
			{
				FE_PRINT("[unrar_ptr 0x%x::~unrar_ptr]delete  p = 0x%x\n", this, p);
				delete p;
				p = (unrar_t*)NULL;
			}
		}
...
{
	*impl_out = (unrar_t*)NULL;
	
	unrar_ptr ptr;
	ptr.p = new unrar_t;

	FE_PRINT("[unrar_open_custom] new ptr.p:0x%x, Len: %d\n",ptr.p,sizeof(unrar_t));
...
void unrar_close( unrar_t* ar )
{
	if ( ar )
	{
		FE_PRINT("[unrar_close]delete  ar = 0x%x\n", ar);
		delete ar;
		ar = (unrar_t*)NULL;
	}
}
...
typedef void (*close_file_fun)( void* );
		...
		close_file_fun close_file = p->close_file;
		p->close_file = (close_file_fun)NULL;
		...
		FE_PRINT("[reopen] new p:0x%x, Len: %d\n",p,sizeof(unrar_t));
