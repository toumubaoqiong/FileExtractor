...
ModelPPM::ModelPPM()
{
	MinContext=(struct PPM_CONTEXT *)NULL;
	MaxContext=(struct PPM_CONTEXT *)NULL;
	MedContext=(struct PPM_CONTEXT *)NULL;
}
...
{
	int i, k, m;
	memset((char*)CharMask,0,sizeof(CharMask));
	SubAlloc.InitSubAllocator();
	InitRL=-(MaxOrder < 12 ? MaxOrder:12)-1;
	MinContext = MaxContext = (PPM_CONTEXT*) SubAlloc.AllocContext();
	MinContext->Suffix=(PPM_CONTEXT*)NULL;
	OrderFall=MaxOrder;
	MinContext->U.SummFreq=(MinContext->NumStats=256)+1;
	FoundState=MinContext->U.Stats=(STATE*)SubAlloc.AllocUnits(256/2);
	for (RunLength=InitRL, PrevSuccess=0, i=0; i < 256;i++)
	{
		MinContext->U.Stats[i].Symbol=(byte)i;
		MinContext->U.Stats[i].Freq=1;
		MinContext->U.Stats[i].Successor=(PPM_CONTEXT*)NULL;
	}
	...
	for (i=0;i < 128;i++)
		for (k=0;k < 8;k++)
			for (m=0;m < 64;m += 8)
				BinSumm[i][k+m]=(ushort)( BIN_SCALE - InitBinEsc[k]/(i+2) );
	for (i=0;i < 25;i++)
		for (k=0;k < 16;k++)
...
*/
	{
		...
		NS2BSIndx[0]=2*0;
		NS2BSIndx[1]=2*1;
		memset( (char*)( NS2BSIndx + 2 ),2*2,9);
		memset( (char*)( NS2BSIndx + 11 ),2*3,256-11);
		for (i=0;i < 3;i++)
			NS2Indx[i]=(byte)i;
		...
		memset( (char*)HB2Flag,0,0x40);
		memset( (char*)( HB2Flag + 0x40 ),0x08,0x100-0x40);
...
void PPM_CONTEXT::rescale(ModelPPM *Model)
{
	...
	U.SummFreq = (ushort)(p->Freq=(byte)( (p->Freq+Adder) >> 1) );
	do
	{
		EscFreq -= (++p)->Freq;
		U.SummFreq = U.SummFreq + (ushort)( p->Freq=(byte)( (p->Freq+Adder) >> 1 ) );
	...
	if (p->Freq == 0)
	{
		...
		EscFreq += i;
		if ( ( NumStats = NumStats - (ushort)i ) == 1 )
		{
			STATE tmp=*U.Stats;
			do
			{
	...
	U.SummFreq = U.SummFreq + (ushort)( EscFreq -= (EscFreq >> 1) );
...
inline void ModelPPM::UpdateModel()
{
	...
	if (fs.Freq < MAX_FREQ/4 && (pc=MinContext->Suffix) != (PPM_CONTEXT*)NULL)
	{
		if (pc->NumStats != 1)
		{
	...
	*SubAlloc.pText++ = fs.Symbol;
	...
	if ( fs.Successor )
	{
		if ((byte*) fs.Successor <= SubAlloc.pText &&
				(fs.Successor=CreateSuccessors(false,p)) == (PPM_CONTEXT*)NULL)
			goto RESTART_MODEL;
		if ( !--OrderFall )
		{
	...
	for (pc=MaxContext;pc != MinContext;pc=pc->Suffix)
	{
		if ((ns1=pc->NumStats) != 1)
		{
			...
			pc->U.SummFreq = pc->U.SummFreq + (ushort)( ( 2*ns1 < ns )+2*( ( 4*ns1 <= ns ) & ( (uint)pc->U.SummFreq <= 8*ns1) ) );
		}
		else
		{
			...
			*p=pc->OneState;
			...
			if (p->Freq < MAX_FREQ/4-1)
				...
				p->Freq  = MAX_FREQ-4;
			pc->U.SummFreq=(ushort)( p->Freq + InitEsc + ( ns > 3 ) );
		}
		...
		else
		{
			...
			pc->U.SummFreq = pc->U.SummFreq + (ushort)cf;
		}
		p=pc->U.Stats+ns1;
...
inline void PPM_CONTEXT::decodeBinSymbol(ModelPPM *Model)
{
	...
	{
		Model->FoundState=&rs;
		rs.Freq = rs.Freq + ( ( rs.Freq < 128 ) ? 1 : 0 );
...
inline bool PPM_CONTEXT::decodeSymbol1(ModelPPM *Model)
{
  ...
  if (count>=(int)Model->Coder.SubRange.scale)
	...
	if (count < (HiCnt=p->Freq))
	{
		...
		(Model->FoundState=p)->Freq=(byte)(HiCnt += 4);
		U.SummFreq += 4;
		if (HiCnt > MAX_FREQ)
			rescale(Model);
		Model->Coder.SubRange.LowCount=0;
		...
		if (Model->FoundState==(STATE*)NULL)
			return(false);
	Model->PrevSuccess=0;
	i=NumStats-1;
...
inline bool PPM_CONTEXT::decodeSymbol2(ModelPPM *Model)
{
  ...
  if (count>=(int)Model->Coder.SubRange.scale)
	...
	else
	{
		...
		psee2c->Summ = psee2c->Summ + (ushort)Model->Coder.SubRange.scale;
