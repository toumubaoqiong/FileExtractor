...
unrar_err_t CmdExtract::ExtractCurrentFile( bool SkipSolid, bool check_compatibility_only )
{
	...
	else
	{
		...
		if ( !Unp )
		{
			...
			FE_PRINT("[CmdExtract 0x%x::ExtractCurrentFile] new Unp:0x%x, Len: %d\n",
				this,
				Unp,
				sizeof(Unpack)
				);
...
#endif
			Unp->DoUnpack(Arc.NewLhd.UnpVer, ( Arc.NewLhd.Flags & LHD_SOLID ) ? true : false );
	}
...
void CmdExtract::UnstoreFile(Int64 DestUnpSize)
{
	...
	while (1)
	{
		...
		if (Code==0 || (int)Code==-1)
			break;
		Code = Code < (unsigned int)DestUnpSize ? Code : int64to32(DestUnpSize);
