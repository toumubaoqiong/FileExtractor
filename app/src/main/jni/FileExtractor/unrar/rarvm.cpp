...
{
	memcpy( (char*)R,(char*)Prg->InitR, sizeof(Prg->InitR) );
	unsigned int GlobalSize=Min(Prg->GlobalData.Size(),VM_GLOBALMEMSIZE);
	if (GlobalSize)
		memcpy( (char*)( Mem + VM_GLOBALMEMADDR ), (char*)&Prg->GlobalData[0],GlobalSize);
	unsigned int StaticSize=Min(Prg->StaticData.Size(),VM_GLOBALMEMSIZE-(int)GlobalSize);
	if (StaticSize)
		memcpy( (char*)( Mem + VM_GLOBALMEMADDR + GlobalSize ), (char*)&Prg->StaticData[0],StaticSize);
  ...
  if (Prg->CmdCount>0 && !ExecuteCode(PreparedCode,Prg->CmdCount))
	...
	if (DataSize!=0)
	{
		...
		memcpy( (char*)&Prg->GlobalData[0], (char*)&Mem[VM_GLOBALMEMADDR],DataSize+VM_FIXEDGLOBALSIZE);
	}
}
...
bool RarVM::ExecuteCode(VM_PreparedCommand *PreparedCode,int CodeSize)
{
	...
	while (1)
	{
		...
		switch(Cmd->OpCode)
		{
			...
			case VM_PUSH:
				R[7]-=4;
				SET_VALUE(false,(uint *)&Mem[R[7]&VM_MEMMASK], (int)( GET_VALUE(false,Op1) ) );
				break;
			case VM_POP:
				SET_VALUE(false,Op1, (int)( GET_VALUE(false,(uint *)&Mem[R[7] & VM_MEMMASK]) ) );
				R[7]+=4;
				break;
			case VM_CALL:
				R[7]-=4;
			...
			case VM_NEG:
				{
					uint Result=UINT32( -(int)( GET_VALUE(Cmd->ByteMode,Op1) ) );
...
#ifdef VM_OPTIMIZE
			case VM_NEGB:
				SET_VALUE(true,Op1,-(int)( GET_VALUE(true,Op1) ) );
				break;
			case VM_NEGD:
				SET_VALUE(false,Op1,-(int)( GET_VALUE(false,Op1) ) );
				break;
#endif
			case VM_PUSHA:
				{
			...
			case VM_POPA:
				{
					const int RegCount=sizeof(R)/sizeof(R[0]);
					for ( uint I=0, SP=R[7]; I<(uint)RegCount;I++,SP+=4)
			...
			case VM_RET:
				if (R[7]>=VM_MEMSIZE)
					return(true);
				SET_IP( (int)( GET_VALUE( false,(uint *)&Mem[R[7] & VM_MEMMASK] ) ) );
				R[7]+=4;
				continue;
...
{
	InitBitInput();
	memcpy( (char*)InBuf, (char*)Code, Min(CodeSize,BitInput::MAX_SIZE) );
	...
	if (XorSum==Code[0]) // VM code is valid if equal.
	{
		...
		if (DataFlag&0x8000)
		{
			unsigned int DataSize=ReadData(*this)+1;
			for (unsigned int I=0;InAddr<CodeSize && I<DataSize;I++)
			{
				Prg->StaticData.Add(1);
				Prg->StaticData[I]=(byte)( fgetbits()>>8 );
				faddbits(8);
			}
		}
		...
		while (InAddr<CodeSize)
		{
			...
			CurCmd->Op1.Addr=CurCmd->Op2.Addr=(uint *)NULL;
			if (OpNum>0)
			{
...
void RarVM::SetMemory(unsigned int Pos,byte *Data,unsigned int DataSize)
{
	if (Pos<VM_MEMSIZE && Data!=Mem+Pos)
		memmove( (char*)( Mem + Pos ), (char*)Data, Min(DataSize,VM_MEMSIZE-Pos) );
}
...
void RarVM::Optimize(VM_PreparedProgram *Prg)
{
	...
	unsigned int CodeSize=Prg->CmdCount;
	...
	for (unsigned int I=0;I<CodeSize;I++)
	{
		...
		for (unsigned int J=I+1;J<CodeSize;J++)
		{
		...
		case VMSF_E8E9:
			{
				...
				if ((unsigned int)DataSize>=VM_GLOBALMEMADDR || DataSize<4)
					break;
		...
		case VMSF_ITANIUM:
			{
				...
				if ((unsigned int)DataSize>=VM_GLOBALMEMADDR || DataSize<21)
					break;
		...
		case VMSF_DELTA:
			{
				...
				if ((unsigned int)DataSize>=VM_GLOBALMEMADDR/2)
					break;
		...
		case VMSF_RGB:
			{
				...
				SET_VALUE(false,&Mem[VM_GLOBALMEMADDR+0x20],DataSize);
				if ((unsigned int)DataSize>=VM_GLOBALMEMADDR/2 || PosR<0)
					break;
				for (int CurChannel=0;CurChannel<Channels;CurChannel++)
				{
				...
				for (int CurChannel=0;CurChannel<Channels;CurChannel++)
				{
					...
					for (int I=CurChannel,ByteCount=0;I<DataSize;I+=Channels,ByteCount++)
					{
						...
						if ((ByteCount & 0x1f)==0)
						{
							...
							Dif[0]=0;
							for ( int J = 1; J < (int)( sizeof(Dif)/sizeof(Dif[0]) ); J++ )
							{
								if (Dif[J]<MinDif)
								{
									MinDif=Dif[J];
