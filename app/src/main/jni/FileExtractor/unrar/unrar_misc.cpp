...
extern "C" {
	static unrar_err_t extract_write( void* user_data, const void* in, int count )
	{
		...
		if ( remain > 0 )
		{
			...
			memcpy( (char*)p->out, (char*)in, count );
			p->out += count;
		}
...
{
	return (count == (int)p->Arc.NewLhd.UnpSize && p->Unp && in == p->Unp->window_wrptr());
}
...
extern "C" {
	static unrar_err_t extract_mem( void* data, void const* in, int count )
	{
		...
		if ( !p->own_data_ )
		{
			...
			p->own_data_ = malloc( (int)( size ? size : 1 ) );
			if ( !p->own_data_ )
		...
		memcpy( (char*) p->data_, (char*)in, count );
