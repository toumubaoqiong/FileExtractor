...
{
	int i, k;
	memset( (char*)FreeList, 0, sizeof(FreeList) );
	pText=HeapStart;
	...
	for (k++;i < N1+N2+N3+N4;i++,k += 4)
		Indx2Units[i]=(byte)k;
	for (GlueCount=0, k=i=0;k < 128;k++)
	{
		i += (Indx2Units[i] < (byte)(k+1));
		Units2Indx[k]=(byte)i;
	}
}
...
void* SubAllocator::ExpandUnits(void* OldPtr,int OldNU)
{
	...
	if ( ptr )
	{
		memcpy( (char*)ptr, (char*)OldPtr, U2B(OldNU) );
...
void* SubAllocator::ShrinkUnits(void* OldPtr,int OldNU,int NewNU)
{
	...
	if ( FreeList[i1].next )
	{
		...
		memcpy( (char*)ptr, (char*)OldPtr,U2B(NewNU) );
		InsertNode(OldPtr,i0);
		return ptr;
	}
	else
